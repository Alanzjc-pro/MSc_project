...
void estimate_ISI_encode(int inputs[Channel], int rem[Channel],int output[subChannel]){
     ...
     Pack_output: for(int i = 0;i< subChannel;i++){
    	 a = 0x0000000F & int(ISIquan[8*i].to_int());
    	 b = 0x000000F0 & int(ISIquan[8*i+1].to_int() << 4);
    	 c = 0x00000F00 & int(ISIquan[8*i+2].to_int() << 8);
    	 d = 0x0000F000 & int(ISIquan[8*i+3].to_int() << 12);
    	 e = 0x000F0000 & int(ISIquan[8*i+4].to_int() << 16);
    	 f = 0x00F00000 & int(ISIquan[8*i+5].to_int() << 20);
    	 g = 0x0F000000 & int(ISIquan[8*i+6].to_int() << 24);
    	 h = 0xF0000000 & int(ISIquan[8*i+7].to_int() << 28);
