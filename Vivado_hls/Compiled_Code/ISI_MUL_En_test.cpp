	...
	for(int i = 0; i < size;i++){
		...
		if(t == bin * count){
			...
			for(int i = 0; i<subChannel;i++){
				ISI_final[8*i][count-1]   = (int)(outputs[i] & 0x0000000F);
				ISI_final[8*i+1][count-1] =((int)(outputs[i] & 0x000000F0) >> 4);
				ISI_final[8*i+2][count-1] = ((int)(outputs[i] & 0x00000F00) >> 8);
				ISI_final[8*i+3][count-1] = ((int)(outputs[i] & 0x0000F000) >> 12);
				ISI_final[8*i+4][count-1] = ((int)(outputs[i] & 0x000F0000) >> 16);
				ISI_final[8*i+5][count-1] = ((int)(outputs[i] & 0x00F00000) >> 20);
				ISI_final[8*i+6][count-1] = ((int)(outputs[i] & 0x0F000000) >> 24);
				ISI_final[8*i+7][count-1] = ((int)(outputs[i] & 0xF0000000) >> 28);
